<template>
    <div :id="docName">
        <TemplateBase>
            <template v-slot:header>
                ...
                <div :class="cardClass" class="search-card">
                    <SearchCode :search-data="searchData" :common-code="commonCode" @click:search="goSearch" />
                </div>
                <!-- /Search -->
            </template>
            <template v-slot:body>
                ...
                <div :class="cardClass">
                    ...
                    />
                    <DataListCode :data-list="reqList" :common-code="commonCode" @click:detail="goDetail" />
                </div>
                <div>
                    <Paging
                        v-if="paging === 'page'"
        ...
        <BasePopup
            ...
            "
        >
            <template v-slot:title>{{ t('voca.code.details.default') }}</template>
            <template v-slot:msg>
                <CodeDetail
...
import CodeDetail from '@/components/common/mng/code/CodeDetail.vue';
...
import CommanderCode from './include/CommanderCode.vue';
import DataListCode from './include/DataListCode.vue';
import SearchCode from './include/SearchCode.vue';
...
const { svcType, tabIdx, currentLang, commonCode, paging, compPagePerData, getDateFormat } = commonVar({
    props: props,
    searchFunc: goSearch
});
...
const detailId = 'CodeDetail';
const detailModal = ref(null as any);

const listCnt = ref(0);
const reqList = ref([] as any);

const detailInfo = ref({ use_yn: 'Y' } as any);
...
const searchData = reactive({
    ...
    page: 1,
    sort: 'grp_cd asc, sort_order asc',
...
function goSearch(data?: number | { [x: string]: any }) {
    ...
    console.log('goSearch', data, isAxiosFlag.value, paging.value, reqList.value.length);
...
function getListCnt() {
    if (!isAxiosFlag.value) {
        ...
        // gProp?.axios
        //     .post(commonts.getUrl(gProp, '/management/code/code/cnt'), searchData, getAxiosOptions('DATA', {}))
...
function goList() {
    if (!isAxiosFlag.value) {
        ...
        // gProp?.axios
        // 	.post(commonts.getUrl(gProp, "/management/code/code/list"), searchData, getAxiosOptions("DATA", {}))
        // 	.then(function (response: any) {
...
function goDetail(data: string | { [x: string]: any }) {
    ...
    // commonts.showModal(refCmCdDetail?.value?.$el);
    commonts.showModal(detailModal.value, 200);
}

function goExcelExport() {
    doExcelExport('/management/code/code/export/excel', searchData);
}

function goExcelImport() {
    goUpload('/management/code/code/import/excel', 'EXCEL_FILE_UPLOAD', goSearch);
}
...
onBeforeMount(() => {
    // gProp?.axios.all([commonts.getCmCds(gProp, ['CMM900', 'CMM906'], '')]).then(
    ...
    //     })
    // )
    // const grpCds = ['CMM900', 'CMM901', 'CMM902', 'CMM904'];
    const grpCds = ['CMM900', 'CMM902'];
