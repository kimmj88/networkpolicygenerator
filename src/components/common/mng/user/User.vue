<template>
                ...
                <div class="px-3 pb-3">
                    <div class="table table-sort w-full">
                        <div class="flex flex-wrap justify-between items-center min-h-[2.5rem] font-bold">
                            <span
                                data-id="svcType"
                                class="header basis-[14%] text-center"
                                v-if="svcType === 'ALL'"
                            ...
                            <span
                                data-id="userId"
                                class="header basis-[19%] text-center"
                            ...
                            <span
                                data-id="useYn"
                                class="header basis-[14%] text-center"
                                :title="t('voca.use.yn.default')"
                            >
                                <em class="sort"></em>
                                {{ t('voca.use.yn.default') }}
                            </span>
                            <span
                                data-id="createId"
                        ...
                        <div class="body" v-if="authRead">
                            <template v-if="reqList.length > 0">
                                ...
                                >
                                    <span class="basis-[14%] text-center" :title="d.svc_type" v-if="svcType === 'ALL'">
                                        {{ d.svc_type }}
                                    </span>
                                    <span class="basis-[19%] cursor-pointer" :title="d.user_id" @click="goDetail(d)">{{
                                        d.user_id
                                    }}</span>
                                    <span class="basis-[19%]" :title="d.user_nm">{{ d.user_nm }}</span>
                                    <span
                                        class="basis-[14%]"
                                        :title="
                                            d.use_yn?.toLowerCase() === undefined
                                                ? ''
                                                : $t('voca.use.' + d.use_yn?.toLowerCase() + '.default')
                                        "
                                    >
                                        {{
                                            d.use_yn?.toLowerCase() === undefined
                                    ...
                                    <span class="basis-[19%] text-center" :title="getDateFormat(d.create_time)">{{
    ...
    <BasePopup
        ...
        "
    >
        <template v-slot:title>{{ t('voca.user.details.default') }}</template>
        <template v-slot:msg>
            <UserDetail
...
import UserDetail from '@/components/common/mng/user/UserDetail.vue';
