                ...
                <div class="p-0">
                    <form class="form-horizontal">
                        <div class="container-fluid">
                            <!-- <div class="te">Device OTP QR cod scan</div> -->
...
function makeQr() {
    if (updateFlag.value === false) {
        let url = commonts.getUrl(gProp, '/api/webui/mfa/otp/registration');
...
function doCheckQr() {
    let url = commonts.getUrl(gProp, '/api/webui/mfa/otp/registrationret');

    gProp?.axios
        .post(url, requestId.value, getAxiosOptions('DATA', {}))
        .then(function () {
            commonts.notificationShow(gProp, 'success', t('msg.isign.success_regist_otp'));
            closeModal();
        })
        .catch((error: any) => {
            commonts.notificationShow(gProp, 'error', t('msg.isign.fail_regist_otp'));
            commonts.debugLog(gProp, error.response);
            failCnt.value++;
            if (failCnt.value >= 5) {
                failCnt.value = 0;
...
onUpdated(() => {
    ...
    if (props.userInfo?.regOtpflag === true) {
        makeQr();
    }
