            ...
            <div class="card search-card">
                <div class="p-0">
                    <div class="justify-content-xl-around items-center ml-0 grid-container">
                        ...
                        <div class="item ellipsis">
                            <span :title="t('voca.log.category.default')">
                                {{ t('voca.log.category.default') }}
                            </span>
                        </div>
                        <div class="item">
                            <v-select
                                class="form-control rounded single w-100 p-0"
                                v-model="searchData.log_type"
                                :value="searchData.log_type"
                                @update:modelValue="goSearch(1)"
                                :options="commonCode?.cmm901"
                        ...
                        <div class="item ellipsis">
                            <span :title="t('voca.admin.id.default')">
                                {{ t('voca.admin.id.default') }}
                            </span>
                        </div>
                        <div class="item">
                            <input
                        ...
                        <div class="item ellipsis">
                            <span :title="t('voca.admin.name.default')">
                                {{ t('voca.admin.name.default') }}
                            </span>
                        </div>
                        <div class="item">
                            <input
                                ...
                                id="user_nm"
                                class="form-control border-1 string"
                                v-model.trim.lazy="searchData.user_nm"
                                @keyup.enter="goSearch(1)"
                                :placeholder="t('msg.info.input.default')"
                ...
                <div class="card-body">
                    ...
                    <div class="table table-sort w-full">
                        <div class="flex justify-between content-between">
                            ...
                            >
                            <span class="header basis-1/6 text-center" :title="t('voca.admin.name.default')">{{
                                t('voca.admin.name.default')
                            }}</span>
                            <span class="header basis-1/6 text-center" :title="t('voca.connection.ip.default')">{{
                                t('voca.connection.ip.default')
                            }}</span>
                            <span class="header basis-1/6 text-center" :title="t('voca.log.category.default')">{{
                                t('voca.log.category.default')
                            }}</span>
                            <span class="header basis-1/6 text-center" :title="t('voca.user.id.default')">{{
                                t('voca.user.id.default')
                            }}</span>
                            <span class="header basis-1/6 text-center" :title="t('voca.details.default')">{{
                                t('voca.details.default')
                            }}</span>
                            <span class="header basis-1/6 text-center" :title="t('voca.access.time.default')">{{
                                t('voca.access.time.default')
                            }}</span>
                        </div>
                        ...
                        <div class="body" v-if="authRead">
                            <template v-if="reqList.length > 0">
                                ...
                                >
                                    ...
                                    <span class="basis-1/6" :title="d.user_nm">{{ d.user_nm }}</span>
                                    <span class="basis-1/6" :title="d.access_ip">{{ d.access_ip }}</span>
                                    <span class="basis-1/6" :title="t('msg.account.' + d.log_type)">{{
                                        t('msg.account.' + d.log_type)
                                    }}</span>
                                    <span
                                        class="basis-1/6"
                                        :title="d.detail_info"
                                        v-html="d.detail_info.split(' ').join('&nbsp;').split('\n').join('<br/>')"
                                    ></span>
                                    <span class="basis-1/6 text-center" :title="getDateFormat(d.access_time)">{{
                                        getDateFormat(d.access_time)
                                    }}</span>
                                </div>
                            </template>
                            <template v-else>
...
function getListCnt() {
    if (!isAxiosFlag.value) {
        ...
        gProp?.axios
            .post(commonts.getUrl(gProp, '/log/account/cnt'), searchData, getAxiosOptions('DATA', {}))
...
function goList() {
    if (!isAxiosFlag.value) {
        ...
        gProp?.axios
            .post(commonts.getUrl(gProp, '/log/account/list'), searchData, getAxiosOptions('DATA', {}))
            .then(function (response: any) {
