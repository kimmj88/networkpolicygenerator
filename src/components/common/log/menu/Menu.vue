                ...
                <div class="p-0">
                    <div class="justify-content-xl-around items-center ml-0 grid-container">
                        <div class="item ellipsis">
                            <span :title="t('voca.use.time.default')">
                                {{ t('voca.use.time.default') }}
                            </span>
                        </div>
                        <div class="item">
                            <datepicker
                ...
                <div class="card-body">
                    ...
                    <div class="table table-sort w-full">
                        <div class="flex justify-between content-between">
                            ...
                            <span class="header basis-1/12 text-center" :title="t('voca.connection.ip.default')">{{
                                t('voca.connection.ip.default')
                            }}</span>
                            <span class="header basis-1/12 text-center" :title="t('voca.menu.name.default')">{{
                                t('voca.menu.name.default')
                            }}</span>
                            <span class="header basis-1/12 text-center" :title="t('voca.menu.id.1.default')">{{
                                t('voca.menu.id.1.default')
                            }}</span>
                            <span class="header basis-1/12 text-center" :title="t('voca.menu.id.2.default')">{{
                                t('voca.menu.id.2.default')
                            }}</span>
                            <span class="header basis-1/12 text-center" :title="t('voca.menu.id.3.default')">{{
                                t('voca.menu.id.3.default')
                            }}</span>
                            <span class="header basis-1/12 text-center" :title="t('voca.menu.id.4.default')">{{
                                t('voca.menu.id.4.default')
                            }}</span>
                            <span class="header basis-1/12 text-center" :title="t('voca.menu.id.5.default')">{{
                                t('voca.menu.id.5.default')
                            }}</span>
                            <span class="header basis-1/12 text-center" :title="t('voca.menu.url.default')">{{
                                t('voca.menu.url.default')
                            }}</span>
                            <span class="header basis-1/12 text-center" :title="t('voca.use.time.default')">{{
                                t('voca.use.time.default')
                            }}</span>
                        </div>
                        ...
                        <div class="body" v-if="authRead">
                            <template v-if="reqList.length > 0">
                                ...
                                >
                                    ...
                                    <span class="basis-1/12" :title="d.access_ip">{{ d.access_ip }}</span>
                                    <span class="basis-1/12" :title="getMenuName(d)">{{ getMenuName(d) }}</span>
                                    <span class="basis-1/12" :title="d.menu_id1">{{ d.menu_id1 }}</span>
                                    <span class="basis-1/12" :title="d.menu_id2">{{ d.menu_id2 }}</span>
                                    <span class="basis-1/12" :title="d.menu_id3">{{ d.menu_id3 }}</span>
                                    <span class="basis-1/12" :title="d.menu_id4">{{ d.menu_id4 }}</span>
                                    <span class="basis-1/12" :title="d.menu_id5">{{ d.menu_id5 }}</span>
                                    <span class="basis-1/12" :title="d.menu_url">{{ d.menu_url }}</span>
                                    <span class="basis-1/12 text-center" :title="getDateFormat(d.create_time)">{{
...
function getListCnt() {
    if (!isAxiosFlag.value) {
        ...
        gProp?.axios
            .post(commonts.getUrl(gProp, '/log/menu/cnt'), searchData, getAxiosOptions('DATA', {}))
...
function goList() {
    if (!isAxiosFlag.value) {
        ...
        gProp?.axios
            .post(commonts.getUrl(gProp, '/log/menu/list'), searchData, getAxiosOptions('DATA', {}))
            .then(function (response: any) {
...
function getMenuName(obj: { [x: string]: any }) {
    let menuId = obj.menu_id5 !== '' && obj.menu_id5 !== undefined ? obj.menu_id5 : obj.menu_id4;
    if (menuId === '' || menuId === undefined) {
        menuId = obj.menu_id3;
    }

    let retName = '';
    menuList?.value?.some((val: { [x: string]: any }) => {
        if (menuId === val.menu_id) {
            retName = val.menu_nm;
            return true;
        }
    });
...
async function getMenuList() {
    ...
    const res = await gProp?.axios
        .post(commonts.getUrl(gProp, '/management/menu/list'), menuSearchData, getAxiosOptions('DATA', {}))
        .then(function (response: any) {
...
function goExcelExport() {
    doExcelExport('/log/menu/export/excel', searchData);
}
...
onBeforeMount(() => {
    gProp?.axios.all([getMenuList()]).then(
        gProp?.axios.spread((menuListData: Array<{ [x: string]: any }>) => {
            menuList.value = JSON.parse(JSON.stringify(menuListData));
            nextProcess();
        })
    );
});
